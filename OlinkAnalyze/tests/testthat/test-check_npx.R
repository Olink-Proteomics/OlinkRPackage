...
# Test get_check_npx (via run_check_npx interface) ----

test_that(
  "get_check_npx - works - no check_log",
  {
    ...
    expect_message(
      object = get_check_npx(df = test_npx_data1),
...
test_that(
  "get_check_npx - works - check_log is all correct",
  {
    ...
    expect_equal(
      object = get_check_npx(df = npx_data1,
...
test_that(
  "get_check_npx - error - check_log has no names",
  {
    test_check_log <- check_npx(df = npx_data1) |>
      suppressWarnings() |>
      suppressMessages()
    ...
    expect_error(
      object = get_check_npx(df = npx_data1,
                             check_log = test_check_log),
      regexp = "`check_log` must be a named list"
    )
  }
)

test_that(
  "get_check_npx - error - check_log misses some names",
  {
    test_check_log <- check_npx(df = npx_data1) |>
      suppressWarnings() |>
      suppressMessages()
...
test_that(
  "get_check_npx - works - uses provided check_log when df has none",
  {
    ...
    result <- get_check_npx(df = df_tbl, check_log = check_log_result)
...
test_that(
  "get_check_npx - works - runs check_npx when df has no check_log and none provided",
  {
    df_tbl <- dplyr::tibble(
      SampleID = LETTERS[1L:4L],
      OlinkID = paste0("OID1234", seq(1L:4L)),
    ...
    expect_message(
      object = {
        result <- get_check_npx(df = df_tbl) |> suppressWarnings()
      },
      regexp = "check_log.*not provided.*Running.*check_npx"
    )

    expect_true(object = is.list(result))
    expect_true(object = all(check_npx_lst_names %in% names(result)))
  }
)
...
test_that(
  "get_check_npx - error - invalid check_log provided for plain tibble",
  {
    ...
    expect_error(
      object = get_check_npx(df = df_tbl, check_log = list(a = 1L)),
      regexp = "missing from"
    )
  }
)
...
test_that(
  "get_check_npx - prefers embedded check_log over provided check_log argument",
  {
    ...
    result <- get_check_npx(df = obj, check_log = list(a = 1L))
