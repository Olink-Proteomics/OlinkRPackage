...
normalization_results.bridged <- olink_normalization(
  df1 = {
    npx_data1 |> mutate(Normalization = "Intensity")
  }, # adding Normalization to avoid warning
  df2 = {
    npx_data2 |> mutate(Normalization = "Intensity")
  }, # adding Normalization to avoid warning
...
npxBridged <- olink_normalization(
  df1 = npx_data_format221010,
  df2 = npx_data_format221010.project2,
  overlapping_samples_df1 = overlap_samples,
  reference_project = "P1"
)

npxBridged_proj2ref <- olink_normalization(
  df1 = npx_data_format221010,
  df2 = npx_data_format221010.project2,
  overlapping_samples_df1 = overlap_samples,
...
test_that("olink_normalization works", {
  expect_warning(expect_warning(olink_normalization(
    df1 = npx_df_test,
    df2 = npx_df2,
  ...
  expect_error(olink_normalization(
    df1 = {
      npx_data1 |> mutate(Normalization = "Intensity")
    },
    df2 = {
  ...
  expect_error(olink_normalization(
    ...
    overlapping_samples_df1 = c("B64", "B36", "A77", "B7", "A24", "A49", "B76")
...
test_that("Different columns in dfs can be normalized", {
  mismatch_col_norm <- suppressWarnings({
    olink_normalization(
      df1 = npx_df_test,
      df2 = npx_df2,
      overlapping_samples_df1 = overlap_samples2,
