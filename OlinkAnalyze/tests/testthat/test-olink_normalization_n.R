...
sampleSubset <- c(
  "A6", "A38", "B47", "B22", "A43", "D75", "D79", "C66", "B43",
...
npx_df2 <- OlinkAnalyze::npx_data2 |>
  ...
  ) # adding Normalization to avoid warning

npx_df_test <- OlinkAnalyze::npx_data1 |>
  dplyr::mutate(Sample_type = "Sample") |>
  dplyr::select(
    SampleID, Sample_type, Index, OlinkID, UniProt, Assay,
...
overlap_samples_na <- intersect(
  npx_data_format221010$SampleID,
...
npx_multi_df3 <- npx_data2 |>
  dplyr::filter(!stringr::str_detect(SampleID, "CONTROL_")) |>
  dplyr::mutate(
    SampleID = paste(SampleID, "_mod", sep = ""),
...
npx_multi_df4 <- npx_data1 |>
  dplyr::filter(!stringr::str_detect(SampleID, "CONTROL_")) |>
  dplyr::mutate(
    SampleID = paste(SampleID, "_mod2", sep = ""),
...
overlap_samples_df2_df3 <- list(
  "DF1" = c("A13", "A29", "A30", "A36", "A45", "A46", "A52", "A63", "A71", "A73"),
...
test_that("olink_normalization multi-batch works", {
  ...
  expect_equal(
    {
      normalization_results.complex_n |> nrow()
    },
    14520L
  )
  ...
  expect_equal(
    {
      normalization_results.complex_n |> ncol()
    },
    16L
  )
  ...
  expect_equal(
    {
      normalization_results.complex_n$Project |> unique()
    },
    c(
      "DF1b",
      "DF2b",
  ...
  expect_equal(
    {
      c(
        {
          normalization_results.complex_n$Adj_factor |> min(na.rm = TRUE)
        },
        {
...
test_that("Different columns in dfs can be normalized", {
  # Bridge ----
  mismatch_col_norm <- olink_normalization_bridge(
    project_1_df = npx_df_test,
    project_2_df = npx_df2,
    bridge_samples = list(
      "DF1" = overlap_samples,
