...
lmer_plot_moreProts <- olink_lmer_plot(
  df = npx_data1,
  variable = c("Treatment", "Time"),
  random = "Subject",
  olinkid_list = {
...
test_that("olink_lmer_posthoc works", {
  ...
  expect_error(olink_lmer_posthoc(
    df = npx_data1,
    variable = c("Treatment", "Time"),
    random = "Subject",
    olinkid_list = {
  ...
  expect_warning(olink_lmer_posthoc(
...
lmer_plot_excludedids <- suppressWarnings(olink_lmer_plot(
  df = npx_data_format221010,
