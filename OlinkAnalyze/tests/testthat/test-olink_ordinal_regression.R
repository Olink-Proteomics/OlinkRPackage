...
test_that(
  "olink_ordinal_regression - works - site",
  {
    ...
    expected_res_dim <- c(184L, 10L)
    expected_res_sig_n <- 21L
    expected_res_sig <- dplyr::tibble(
    ...
    expect_equal(
      ...
      expected = expected_res_sig,
      tolerance = 1e-6
    )

    # olink_class ----

    npx_data1_mod_obj <- attach_check_log(
      df = npx_data1_mod,
      out_df = "tibble"
    )
    ...
    expect_no_warning(
      object = expect_no_error(
        object = expect_message(
          object = expect_message(
            object = ord_regs_res_site_obj <- olink_ordinal_regression(
    ...
    expect_identical(
      object = dim(ord_regs_res_site_obj),
      expected = expected_res_dim
    )

    expect_identical(
      object = ord_regs_res_site_obj |>
        dplyr::filter(.data[["Threshold"]] == "Significant") |>
        nrow(),
      expected = expected_res_sig_n
    )
...
test_that(
  "olink_ordinal_regression - works - time",
  {
    ...
    expected_res_dim <- c(184L, 10L)
    expected_res_sig_n <- 0L
    expected_res_top_20 <- dplyr::tibble(
    ...
    expect_no_warning(
      object = expect_no_error(
        object = expect_message(
          object = expect_message(
            object = ord_regs_res_time_obj <- olink_ordinal_regression(
              df = npx_data1_mod_obj,
              variable = "Time"
            ),
            regexp = paste("Variables and covariates converted from character",
    ...
    expect_identical(
      object = dim(ord_regs_res_time_obj),
      expected = expected_res_dim
    )

    expect_identical(
      object = ord_regs_res_time_obj |>
        dplyr::filter(.data[["Threshold"]] == "Significant") |>
        nrow(),
      expected = expected_res_sig_n
    )
  ...
  "olink_ordinal_regression - works - treatment*time",
  {
    ...
    expect_no_warning(
      object = expect_no_error(
        object = expect_message(
          object = expect_message(
            object = ord_regs_res_treat_time_obj <- olink_ordinal_regression(
              df = npx_data1_mod_obj,
              variable = "Treatment:Time"
            ) |>
              dplyr::mutate(
    ...
    expect_equal(
      object = ord_regs_res_treat_time_obj,
      expected = reference_results$ordinal_regression
    )
  }
)
...
test_that(
  "olink_ordinal_regression_posthoc - works - site",
  {
    ...
    expected_res_dim <- c(210L, 9L)
    expected_res_sig_n <- 54L
    expected_res_sig_top23 <- dplyr::tibble(
    ...
    # ordinal regression results and significant OIDs for posthoc testing ----
    ...
    expect_equal(
      ...
      expected = expected_res_sig_top23,
      tolerance = 1e-6
    )
    ...
    expect_no_error(
      object = expect_no_warning(
        object = expect_message(
          object = expect_message(
            object = ord_regs_posthoc_res_site_obj <-
              olink_ordinal_regression_posthoc(
                df = npx_data1_mod_obj,
                variable = "Site",
                olinkid_list = ord_regs_res_site_oid,
    ...
    expect_identical(
      object = dim(ord_regs_posthoc_res_site_obj),
      expected = expected_res_dim
    )
    ...
    expect_no_error(
      object = expect_no_warning(
        object = expect_message(
          object = expect_message(
            object = expect_message(
              object = ord_reg_ph_res_treat_time_obj <-
                olink_ordinal_regression_posthoc(
                  df = npx_data1_mod_obj,
                  variable = "Treatment:Time",
                  olinkid_list = ord_regs_res_treat_time_oid,
