...
set.seed(123)  # reproducible randomization and clustering
...
test_that("OSI errors: using npx_data1 and OSI columns", {
  ...
  osi_data <- osi_data |>
    dplyr::mutate(SampleID = sample(new_ids, size = nrow(osi_data),
                                    replace = FALSE))

  data1 <- OlinkAnalyze::npx_data1 |>
    dplyr::right_join(
      osi_data |>
        dplyr::select(SampleID, OSITimeToCentrifugation,
                      OSIPreparationTemperature, OSISummary, OSICategory),
      by = "SampleID"
    ) |>
    dplyr::mutate(AssayQC = "PASS", SampleType = "SAMPLE", AssayType = "assay")
  ...
  testthat::expect_error(
    olink_umap_plot(df_bad_cat, quiet = TRUE),
    regexp = "Invalid values detected in OSICategory\\. Expected only 0, 1, 2, 3, or 4\\. Found: 7" #nolint
  )

  # ----------------------------
  # OSICategory all NA
  # ----------------------------
  df_cat_all_na <- data1 |> dplyr::mutate(OSICategory = NA)

  testthat::expect_error(
    olink_umap_plot(df_cat_all_na, quiet = TRUE),
    regexp = "All values are NA in OSICategory\\. Please provide at least one non-missing value\\." #nolint
  )
  ...
  testthat::expect_error(
    olink_umap_plot(df_bad_cont_nonnum, quiet = TRUE),
    regexp = paste0(
  ...
  testthat::expect_error(
    olink_umap_plot(df_bad_cont_oor, quiet = TRUE),
    regexp = paste0(
  ...
  # ----------------------------
  df_cont_all_na <- data1 |> dplyr::mutate(OSISummary = NA)

  testthat::expect_error(
    olink_umap_plot(df_cont_all_na, quiet = TRUE),
    regexp = "All values are NA in OSISummary\\. Please provide at least one non-missing value\\." #nolint
  )
  ...
  testthat::expect_no_error(
    olink_umap_plot(data1, quiet = TRUE)
  )
...
test_that("olink_umap_plot works", {
  ...
  dat <- npx_data1 |>
    ...
    dplyr::mutate(
      NPX   = ifelse(SampleID %in% outGroup, NPX + 5, NPX),
      group = ifelse(SampleID %in% outGroup, "A", "B")
    )

  umap_plot <- olink_umap_plot(dat, color_g = "group", quiet = TRUE)

  umap_xy <- umap_plot[[1]]$data |>
    dplyr::select(dplyr::contains("umap"))

  umap_plot.cl <- stats::kmeans(umap_xy, centers = 2) # nolint object_name_linter

  clusterSizes <- table(umap_plot.cl$cluster) # nolint object_name_linter
  smallest_cluster_id <- as.numeric(
    names(clusterSizes)[
      which.min(clusterSizes)
    ]
  )
  outGroup.detected <- names( # nolint object_name_linter
    umap_plot.cl$cluster[
      umap_plot.cl$cluster == smallest_cluster_id
    ]
  )
