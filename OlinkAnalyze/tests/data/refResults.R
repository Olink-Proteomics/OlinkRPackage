...
ordinalRegression_results <- olink_ordinalRegression(
  df = npx_data1,
...
#### lmer posthoc ####
lmer_results_1_posthoc <- olink_lmer_posthoc(
  df = npx_data1,
  variable = c("Treatment", "Time"),
  random = "Subject",
  olinkid_list = {
...
overlap_samples <- intersect(npx_data1$SampleID, npx_data2$SampleID) %>%
  ...
  pull(.)
normalization_results.bridged <- olink_normalization(
  df1 = npx_data1,
  df2 = npx_data2,
...
normalization_results.intensity <- olink_normalization(
  df1 = npx_data1,
  df2 = npx_data2,
...
normalization_results.subset <- olink_normalization(
  df1 = npx_data1,
  df2 = npx_data2,
  overlapping_samples_df1 = npx_data1$SampleID,
...
samples <- sample(
  {
    npx_data1.uniqIDs$SampleID %>% unique()
  },
  size = {
    ceiling((npx_data1.uniqIDs$SampleID %>% unique() %>% length()) * .15)
  }
)
...
procData_missingData <- npxProcessing_forDimRed(
  df = npx_data1.uniqIDs_missingData,
  color_g = "QC_Warning",
  drop_assays = F,
  drop_samples = F,
  verbose = T
