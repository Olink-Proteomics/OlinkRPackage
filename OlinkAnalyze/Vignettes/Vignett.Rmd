...
output: 
  html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{OlinkAnalyze Vignette}
...
Olink Analyze is an R package that provides a versatile toolbox to enable fast and easy handling of Olink NPX data for your proteomics research. OlinkAnalyze provides functions ranging from importing Olink NPX datasets exported from the NPX Manager to various statistical tests and modeling, via different quality control (QC) plot functions. Thereby, providing a convenient pipeline for your Olink NPX data analysis.

# Installation 

The package *devtools* is required for the installation.

```{r, eval=FALSE}
install.packages("devtools")
```

Now you can proceed to installing OlinkAnalyze.
...
* **Panel** _\<chr\>_: Olink Panel that samples ran on. Read more about Olink Panels here: [https://www.olink.com/products/](https://www.olink.com/products/).
...
* **QC_Warning** _\<chr\>_: Indication whether the sample passed Olink QC. Read more here [https://www.olink.com/question/how-is-quality-control-of-the-data-performed/](https://www.olink.com/question/how-is-quality-control-of-the-data-performed/). 
...
* **NPX** _\<dbl\>_: Normalized Protein eXpression, is Olink’s unit of protein expression level in a log<sub>2</sub> scale. The majority of the functions of this package use NPX values for calculations. Read more about NPX here [https://www.olink.com/question/what-is-npx/](https://www.olink.com/question/what-is-npx/).

**NB** There are 5 additional variables in the sample datasets npx_data1 and npx_data2 that include clinical or other information, namely: Subject _\<chr\>_, Treatment _\<chr\>_, Site _\<chr\>_, Time _\<chr\>_, Project _\<chr\>_.

# Preprocessing

## Read NPX data (read_NPX)

The read_NPX function imports an NPX file of wide format that has been exported from Olink NPX Manager and converts the data into the (preferred by R) long format. The wide format is the most common way Olink delivers data, however, for data analysis a long format is preferred. No prior alterations to the output of the NPX Manager should be made for this function to work as expected.
...
* Panel: Olink Panel that samples ran on. Read more about Olink Panels here: [https://www.olink.com/products/](https://www.olink.com/products/).
...
* QC_Warning: Indication whether the sample passed Olink QC. Read more here [https://www.olink.com/question/how-is-quality-control-of-the-data-performed/](https://www.olink.com/question/how-is-quality-control-of-the-data-performed/). 
...
* NPX: Normalized Protein eXpression, is Olink’s unit of protein expression level in a log<sub>2</sub> scale. The majority of the functions of this package use NPX values for calculations. Read more about NPX here [https://www.olink.com/question/what-is-npx/](https://www.olink.com/question/what-is-npx/).

## Randomize sample position (olink_plate_randomizer)

The olink_plate_randomizer function randomly assigne samples to a plate position with the option to keep the same individual on the same plate. Olink don't recommend to force balance based on other clinical variables. 

### Function arguments

+ Manifest: tibble/data frame in long format containing all sample ID's. Sample ID column must be named SampleID.
+ SubjectColumn: (Optional) Column name of the subject ID column. Cannot contain missings. If provided, subjects are kept on the same plate.
+ Groups: (Optional) Currently not implemented! Vector of column names for variables to be balanced across plates.
+ iterations: Number of iterations for fitting subjects on the same plate.
+ available.spots: Default is 88. Number of wells available on each plate. Can also take a vector equal to the number of plates to be used indicating the number of wells available on each plate.
+ seed: Seed to set. Highly recommend setting this for reproducibility.

```{r message=FALSE, eval=FALSE}
olink_plate_randomizer(manifest, 
                       SubjetColumn="SubjectID",
                       seed=111)
```  
...
## Selecting bridge samples (olink_bridgeselector)

The bridge selection function will select a number of bridge samples based on the input data. Bridge samples are used to normalize two dataframes/projects which have been ran at different times. It select samples with good detection, which passes quality control and cover a good range of the data. 

If possible olink recommends 8-16 bridge samples. Bridge sample selection strategy: When running the selector, start at 10% missingness. If there are not enough samples output, increase to 20%. 

### Function arguments

+ df: Tibble/data frame in long format such as produced by the olinkr read_NPX function.
+ sampleMissingFreq: The threshhold for sample wise missingness.
...
olink_normalization(df1 = npx_data1, 
                    ...
                    df1_project_nr = 'Project1',
                    df2_project_nr = 'Project2')
```  
...
Samples with missing variable information or factor levels are excluded from the analysis. Character columns in the input data frame are converted to factors. Both automatic handlings of data from above are announced by a message if the flag *verbose=T*.
...
* return.covariates: Boolean. Default: False. Returns F-test results for the covariates. Note: Adjusted p-values will be NA for covariates.
...
* mean_return: Boolean. If true, returns the mean of each factor level rather than the difference in means (default). Note that no p-value is returned for mean_return = T.
* verbose: Boolean. Default: True. If information about removed samples, factor conversion and final model formula is to be printed to the console.
...
* olinkid_list: Character vector of OlinkID's that should be used for the bocplot. If not specified, all assays in df are used.
* verbose: Boolean. Default: False. Flag indicating if plots shall be printed additionally to assigned to a list variable.
...
**NB** Please note that plots will not appear in the *plots* panel of *Rstudio* if not assigned to a variable and printing it (see sample code above).
...
## Principal compnents analysis (PCA) plot (olink_pca_plot)
...
* color_g: Character value indicating the column name that should be used as fill color. Default QC_Warning.
* plot_index: Boolean. Default FALSE. If FALSE, a point will be plotted for a sample. If TRUE, a sample's unique index number is displayed.
* label_outliers: Boolean. If TRUE, an outlier sample will be labeled by its SampleID.
