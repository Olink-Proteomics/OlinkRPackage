...
bridgeable <- function(data_Explore384,
  ...
  data_mapped <- data_Explore384 |>
    dplyr::inner_join(data_HT, by = c("SampleID", "ids"),
    ...
    dplyr::mutate(range_ht =
                  ...
                  iqr_3k = IQR(.data[["NPX_3k"]]),
                  median_count_3k =
                    median(.data[["Count_3k"]][.data[["Count_3k"]] > 10],
                           na.rm = TRUE),
                  # if count less than 10 in 1 sample that assay is dropped
                  median_count_ht =
                    median(.data[["Count_ht"]][.data[["Count_ht"]] > 10],
                           na.rm = TRUE),
                  # if count less than 10 in 1 sample that assay is dropped
  ...
  not_bridgeable <- data_condensed |>
    dplyr::filter(.data[["range_diff"]] >
                    1, .data[["low_counts_flag"]] ==
                    TRUE, .data[["R2_lm"]] < 0.8) |>
    dplyr::pull(.data[["ids"]]) |>
    unique() |>
    as.data.frame() |>
    dplyr::rename(ids = 1) |>
    tidyr::extract(.data[["ids"]], into = c("OlinkID", "OlinkID_Explore384"),
                   "(.*)_([^_]+)$", remove = TRUE)
  ...
  bridge_table <- e3k_eHT_mapping |>
    dplyr::mutate(Bridgeable =
                    dplyr::if_else(.data[["OlinkID_HT"]] %in%
                                     not_bridgeable$OlinkID, "Not Bridgeable",
                                   "Yes")) |>
    dplyr::select(.data[["OlinkID_HT"]],
                  .data[["OlinkID_E3072"]],
                  .data[["Bridgeable"]]) |>
    dplyr::distinct() |>
    dplyr::inner_join(ks,
