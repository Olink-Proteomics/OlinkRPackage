...
#' @examples
#' \donttest{
#' results <- OlinkAnalyze:::bridgeable(data_Explore384 = OlinkAnalyze:::data_3k,
...
bridgeable <- function(data_Explore384,
  ...
  bridge_table <- e3k_eHT_mapping |>
    dplyr::mutate(Bridgeable = dplyr::if_else(
      .data[["OlinkID_HT"]] %in% not_bridgeable$OlinkID,
      "Not Bridgeable",
      "Yes")) |>
  ...
  bridge_table <- bridge_table |>
    ...
    dplyr::left_join(concat_ids,
                     by = c("OlinkID_HT" = "OlinkID",
                            "OlinkID_E3072" = "OlinkID_Explore384")) |>
    dplyr::rename(
      BridgingRecommendation = .data[["Bridgeable"]],
