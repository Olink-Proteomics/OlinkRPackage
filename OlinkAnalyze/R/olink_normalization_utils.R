...
olink_norm_input_clean_assays <- function(lst_df,
  ...
  return(lst_out) # Add vectors of non overlapping OlinkIDs
}
...
olink_norm_input_assay_overlap <- function(lst_df,
  ...
  if (nrow(oid_combos_miss) > 0L) {
    ...
    if (norm_mode == olink_norm_modes$norm_cross_product) {
      ...
      cli::cli_warn(
        c(
          ...
          "i" = "{cli::qty(oid_removed)} Assay{?s} will be returned without
        adjustment."
        ),
        wrap = FALSE
      )
    ...
    lst_out <- list(
      lst_df = lst_df,
      reference_medians = reference_medians,
      non_overlapping_oid = oid_removed
    )
...
olink_normalization_format <- function(df_norm = df_norm,
  ...
  if (lst_check$norm_mode == olink_norm_modes$bridge) {
    ...
    if (!is.null(lst_check$non_overlapping_oid)) {
      df1_no_overlap <- df1 |>
        dplyr::filter(
          ...
          (.data[["OlinkID"]] %in% lst_check$non_overlapping_oid)
        ) |>
    ...
    df_not_bridgeable <- df_norm |>
      dplyr::filter(
        !(.data[["SampleID"]] %in% nc_pc_sampleids) # remove nc, pc
    ...
    df1_no_overlap <- df1 |>
      dplyr::filter(
        !(.data[["SampleID"]] %in% nc_pc_sampleids) # remove nc, pc
        # keep non-overlapping assays
        & (.data[["OlinkID"]] %in% non_overlapping_oid_split)
      ) |>
      # add missing variables
      dplyr::mutate(
        Project = df1_project_nr,
