...
#'
#' @param data a data frame of results from bridge normalization generated by olink_normalization()
...
bridgeable_plts <- function(data = npx_br_data ,
  ...
  r2_plt <- function(data = npx_br_data_wider, id = id){
    r2_plt <-
      npx_br_data_wider |>
      dplyr::filter(OlinkID %in% id) |>
      ggplot2::ggplot(aes(x = .data[["NPX_Explore HT"]],
                          y = .data[["NPX_Explore 3072"]])) +
      ggplot2::geom_point(color = 'blue', alpha = 0.4) +
      ggplot2::geom_smooth(method = "lm", color = "black") +
      ggpubr::stat_cor(aes(label = ggplot2::after_stat(.data[["rr.label"]])),
                       geom = "label") +
      OlinkAnalyze::set_plot_theme(font = "") +
