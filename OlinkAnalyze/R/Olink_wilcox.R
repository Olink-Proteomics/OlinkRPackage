...
olink_wilcox <- function(df, variable, pair_id, ...) {
  ...
  if (!is.null(removed.sampleids) & length(removed.sampleids) > 0) {
    message(
      "Samples removed due to missing variable levels: ",
  ...
  if (!missing(pair_id)) {
    ...
    if (!is.null(missing.pair) & length(missing.pair) > 0) {
      message(
        "Samples removed due to missing pair ID: ",
  ...
  if (!(number_of_levels == 2)) {
    stop(paste0(
      "The number of levels in the factor needs to be 2. Your factor has ", number_of_levels,
      " levels."
    ))
  }
  ...
  if (number_of_samples_w_more_than_one_level > 0) {
    stop(paste0(
      "There are ", number_of_samples_w_more_than_one_level,
  ...
  if (!missing(pair_id)) {
    ...
    if (!all(ct_pairs$n <= 2)) {
      stop(paste0(
        "Each pair identifier must identify no more than 2 unique samples. Check pairs: ",
    ...
    p.val <- df %>%
      ...
      dplyr::select(all_of(c(
        "OlinkID", "UniProt", "Assay", "Panel", data_type,
        variable, pair_id
      ))) %>%
      tidyr::pivot_wider(
      ...
      dplyr::do(tidy(stats::wilcox.test(
        x = .[[var_levels[1]]],
        y = .[[var_levels[2]]],
        paired = TRUE,
        ...
