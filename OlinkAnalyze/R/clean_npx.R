...
#'
#' @param df A `tibble` or `arrow` object loaded from `read_npx()`,
#' containing a columnmspecified by `check_npx_log$col_names$sample_id`.
#' @param check_npx_log A list returned by `check_npx()`, containing:
...
clean_duplicate_sample_id <- function(
  ...
  cli::cli_alert_warning(
    "Excluding {length(sample_ids_to_remove)} sample{?s} with duplicate
    SampleIDs: {paste(sample_ids_to_remove, collapse = ', ')}"
  )
...
#'
#' @returns A filtered tibble or arrow object with samples that
#' failed QC removed.
#'
#' @examples
#' \dontrun{
...
#' regular expression pattern indicating customer control samples. This is
#' useful for filtering out technical controls or customer control samples based
#' on their SampleID.
#'
#' @author
#' Kang Dong
...
clean_control_sample_id <- function(
  ...
  cli::cli_inform(c(
    "Removing {n_controls} row{?s} of control sample{?s} based on
    sample ID pattern.",
