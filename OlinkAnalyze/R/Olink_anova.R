  ...
  withCallingHandlers(
    {
      ...
      p.val <- df %>%
        ...
        dplyr::do(generics::tidy(car::Anova(stats::lm(as.formula(formula_string),
          data = .,
          contrasts = sapply(fact.vars, function(x) {
              return(contr.sum)
            },
            simplify = FALSE
          )
        ...
        dplyr::mutate(
          Adjusted_pval = ifelse(covariates, NA, Adjusted_pval),
        ...
        dplyr::select(
          Assay, OlinkID, UniProt, Panel, term, df, sumsq,
  ...
  withCallingHandlers(
    {
      ...
      anova_posthoc_results <- df %>%
        ...
        dplyr::group_by(Assay, OlinkID, UniProt, Panel) %>%
        dplyr::do(data.frame(
          emmeans::emmeans(stats::lm(as.formula(formula_string), data = .),
            specs = e_form,
            cov.reduce = function(x) round(c(mean(x), mean(x) + sd(x)), 4),
            infer = c(TRUE, TRUE),
            adjust = post_hoc_padjust_method
          )[[c("contrasts", "emmeans")[1 + as.numeric(mean_return)]]],
      ...
      if (mean_return) {
        anova_posthoc_results <- anova_posthoc_results %>%
          dplyr::select(all_of(c(
            "Assay", "OlinkID", "UniProt", "Panel", "term",
        ...
        anova_posthoc_results <- anova_posthoc_results %>%
          ...
          dplyr::select(tidyselect::any_of(c(
            "Assay", "OlinkID", "UniProt", "Panel", "term", "contrast", effect, "estimate",
