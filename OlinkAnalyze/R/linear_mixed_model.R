...
#'   pull()
#'
#' results_lmer_posthoc <- olink_lmer_posthoc(
#'   df = npx_data1,
  ...
  withCallingHandlers(
    {
      ...
      output_df <- df %>%
        ...
        dplyr::filter(!(OlinkID %in% npxCheck$all_nas)) %>%
        dplyr::group_by(Assay, OlinkID, UniProt, Panel) %>%
        dplyr::group_modify(~ single_posthoc(
          data = .x,
...
#'   pull()
#'
#' list_of_pointrange_plots <- olink_lmer_plot(
#'   df = npx_data1,
#'   variable = c("Time", "Treatment"),
#'   random = c("Subject"),
  ...
  for (i in c(1:length(protein_index))) {
    ...
    lmerplot <- lm.means %>%
      ...
      ggplot2::geom_pointrange(
        ggplot2::aes(
          x = as.factor(!!rlang::ensym(x_axis_variable)),
          y = emmean,
          ymin = conf.low,
          ymax = conf.high,
