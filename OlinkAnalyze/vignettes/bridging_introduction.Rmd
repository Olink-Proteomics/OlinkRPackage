...
npx_before_br <- rbind(npx_1, npx_2) %>% 
  mutate(datatype = ifelse(SampleID %in% c(br_1$SampleID, br_2$SampleID), "bridges", dataset))
...
npx_br_data %>% 
  mutate(Panel = gsub("Olink ", "", Panel)) %>% 
  filter(Project != reference_project) %>% # remove adjustment factors from reference project.
