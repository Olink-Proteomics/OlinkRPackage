...
header-includes:
  ...
  - \pagestyle{fancy}
  - \fancyhead{}
  - \fancyhead[R]{\includegraphics[width=20pt]{../man/figures/small_logo.png}}
  - \fancyhead[L]{\textbf{Introduction to bridging Olink\textsuperscript{®} NPX datasets}}
  # - \fancyhead[R]{\thepage}
...
multi-plate projects. Additionally, the intensity normalization method
assumes that all samples within a project are fully randomized.

Since NPX is a measure of relative quantification, normalization is needed to align NPX values across plates, runs, and batches. Some normalization methods, such as plate control normalization and intensity normalization, can be performed within the Olink NPX software, while other methods, such as subset normalization and bridge normalization, rely on external data to perform normalization and are instead performed using Olink Analyze after data is exported from Olink NPX software.
 
The ideal normalization method for a study will depend on several factors including sample availability, randomization, and similarities to control samples. For example, in Olink Explore, plate control normalization uses the median of the plate controls for adjustment and performs best in assays that are well detected in the plate controls. It does not require external samples or randomization but may not perform as well for samples that are unsimilar to the plate control samples, such as alternative matrices or samples from certain diseases. Intensity normalization, which uses the median of the samples on the plate (excluding the control strip) for adjustment, performs well for any sample type so long as the samples are randomized. If randomization is not optimized, but samples are available, overlapping samples can be used for normalization, known as  overlapping sample reference normalization, bridge normalization, or just bridging.
...
The following tutorial is designed to give an overview of the kinds
of data combining methods that are possible using the Olink® within-product bridging
procedure. For bridging across the NGS-based products see the [Bridging across NGS-based Olink^®^ products](). 

Before starting bridging, it is important to check if the
...
pass QC and have high detectability. In the case of where
...
enough samples in the output, increase to 20%. For alternative matrices and
...
bridge_Samples<- npx_data1 |> 
  ...
  olink_bridgeselector(sampleMissingFreq = 0.1, n = 16)
```
...
f1<- "PCA plot of bridging samples and other samples in npx\\_data1. Control samples are excluded from the PCA plot."

f2 <- "Density plot of NPX distribution in both datasets before bridging."

f3 <- "PCA plot of both datasets before bridging."

f4 <- "Density plot of NPX distribution in both datasets after bridging."

f5 <- "Histogram of adjustment factors in normalized data from Project \"data2\"."

f6 <- "Violin plot of CHL1 in both datasets prior to bridging. Bridge samples are indicated by black points."

f7 <- "Density plot of inter-project CV before and after bridging."

f8 <- "PCA plot of both datasets after bridging."
```
...
The `read_NPX()` function can be used to import an
NPX file as generated by Olink^®^ NPX Software.
...
(`npx_data1` and `npx_data2`) from the **Olink Analyze** package. This
...
data.frame(SampleID = intersect(npx_data1$SampleID, npx_data2$SampleID)) |>
  ...
  kableExtra::kbl(booktabs = TRUE,
      digits = 2,
      caption = "Overlapping Bridge Samples") |>
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = FALSE,
                position = "center", latex_options = "HOLD_position")
```
...
The `olink_normalization_bridge()` function can be used to bridge two
...
The output from the `olink_normalization_bridge()` function is a NPX table
with adjusted NPX value in the column `NPX`, as shown in Table 4.
The `olink_normalization_bridge()` function is a wrapper for and supersedes the
`olink_normalization()` function.

The `olink_normalization_bridge()` function creates a new column `Project` to
...
npx_br_data |> 
  select(OlinkID, Assay, PlateID,	QC_Warning,	LOD, NPX, Project,	Panel, Adj_factor,	SampleID, Adj_factor) |> 
  head(10) |> 
  kableExtra::kbl(booktabs = TRUE,
      digits = 1,
      caption = "Preview of combined datasets after bridging: first 10 rows and selected columns.") |>
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = FALSE, font_size = 20, 
...
Then, summarize the number of assays that have adjustment factors in certain
...
projects is to calculate the Coefficient of Variation (CV) of the control samples across both
...
Olink Explore products. More information on the CV calculation can be found
...
Finally, use a PCA plot to check whether bridging normalization has corrected the batch effect (Figure 8). In the example below, it is
