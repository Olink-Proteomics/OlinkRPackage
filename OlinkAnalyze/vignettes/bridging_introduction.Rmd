...
npx_before_br <- npx_data1 %>%
  ...
  dplyr::mutate(Type = if_else(SampleID %in% overlapping_samples,
                        paste0("Project_1 Bridge"),
                        paste0("Project_1 Sample"))) %>%
  rbind({
    npx_data2 %>%
      filter(!str_detect(SampleID, "CONTROL_SAMPLE")) %>% #Remove control samples %>% 
      mutate(Type = if_else(SampleID %in% overlapping_samples,
                            paste0("Project_2 Bridge"),
                            paste0("Project_2 Sample"))) %>%
...
npx_br_data <- olink_normalization_bridge(project_1_df = npx_1,
                                          ...
                                          project_1_name = "Project_1",
                                          project_2_name = "Project_2",
                                          project_ref_name = "Project_1")
...
npx_br_data %>% 
  dplyr::filter(Project == "Project_2") %>%  # Only looking at Project 2 since project 1 is unadjusted
...
npx_data1 %>%
  mutate(Project = "Project_1") %>%
  bind_rows({
    npx_data2 %>%
      mutate(Project = "Project_2")
  }) %>%
  filter(Assay == "CHL1") %>%
...
new_normalized_data <- npx_br_data %>% 
  dplyr::filter(Project == "Project_2") %>% 
