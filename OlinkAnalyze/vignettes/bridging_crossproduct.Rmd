...
header-includes:
  ...
  - \fancyhead[L]{\textbf{Bridging across NGS-based Olink\textsuperscript{®} products}}
  # - \fancyhead[R]{\thepage}
...
Individual Olink\textsuperscript{®} NPX\textsuperscript{TM} projects are generally normalized using
...
combining data sets from Olink\textsuperscript{®} Explore 3072, Olink\textsuperscript{®} Explore HT,
and Olink\textsuperscript{®} Reveal products.
...
-   **Reference data** – The project data which is being normalized to
    ...
    bridging, the reference project is the Explore HT NPX data or the Reveal NPX data. The
...
Between-product bridging allows the NPX values of an Olink Explore 3072 project
...
tutorial]() or otherwise normalized together
...
```{r, fig.cap= fcap, eval = TRUE, echo = FALSE, out.width="80%"}
knitr::include_graphics(normalizePath("../man/figures/Bridging_schematic.pdf"),
                        error = FALSE)
...
of the Introduction to Bridging tutorial. External control samples often share the same naming
...
```{r eval = FALSE, fig.cap=f3, fig.height= 8, fig.width= 6, out.width="\\linewidth"}
...
```{r echo=FALSE, eval=TRUE, fig.cap=f3, out.width="100%"}
knitr::include_graphics(normalizePath("../man/figures/PCA_btw_product_before.pdf"),
                        error = FALSE)
...
the sections below. Within this function, the b ridging recommendations
...
```{r echo=FALSE, fig.cap=fcap}
knitr::include_graphics(normalizePath("../man/figures/assay_bridgeability.pdf"), 
                        error = FALSE)
...
```{r message=FALSE, echo = FALSE, fig.cap = fcap, fig.pos="H", out.width="85%"}
knitr::include_graphics(
  normalizePath("../man/figures/bridgeable_plt_MedianCenter.pdf"), 
  error = FALSE)

fcap <- "Visualization of an assay's bridgeability criteria as generated by the olink\\_bridgeability\\_plot function."
```
...
method is recommended for bridging. If the fourth criteria is met then
the recommended method is normalization using the median of paired
differences. If any of the first three criteria are met then quantile
smoothing is recommended. If all of the first three criteria are not met
...
normalized NPX values in the Explore HT or the Reveal data will be the same as the
...
try( 
  readRDS(normalizePath("../man/figures/bridging_results.rds")) |> 
    dplyr::select( # select fewer columns for better readability
      SampleID,
      OlinkID,
      Assay,
      Panel,
    ...
    kableExtra::kbl(booktabs = TRUE,
                    linesep = "",
                    digits = 1,
                    caption = "Preview of combined datasets after bridging with between-product formatting argument set to FALSE: first 5 rows and selected columns.") |>
    kableExtra::kable_styling(bootstrap_options = "striped", 
                              full_width = FALSE,
                              font_size = 20, position = "center",
                              latex_options = c("HOLD_position","scale_down")) |> 
    kableExtra::scroll_box(width = "100%") |> 
    kableExtra::column_spec(column = 1:13,width_max = "5em") # change column = 1:13 if needed
)
```
...
try( 
  readRDS(normalizePath("../man/figures/bridging_results.rds")) |> 
    ...
    dplyr::select(  # select fewer columns for readability 
      SampleID,
      OlinkID,
      Assay,
      Panel,
    ...
    kableExtra::kbl(booktabs = TRUE,
                    linesep = "",
        digits = 1,
        caption = "Preview of combined datasets after bridging with between-product formatting argument set to TRUE: first 5 rows and selected columns.") |>
    ...
    kableExtra::scroll_box(width = "100%") |> 
    kableExtra::column_spec(column = 1:10,width_max = "5em")
)
```
...
PCA plots are used to assess the quality of bridging by determining if the
<!-- sample controls (SCs) and  -->
bridging samples appear closer after bridging.

<!-- Two PCAs can be generated, one containing the SCs and one containing the -->
<!-- bridging samples.  -->

Prior to bridging there will be a noticeable
...
```{r pca_pre_sc, echo=FALSE, eval = FALSE}
## Before Bridging
npx_br_data |> 
...
```{r pca_pre_sc_fig, echo=FALSE, fig.cap=f8, message=FALSE, fig.pos="H", eval=FALSE}
## Before Bridging
knitr::include_graphics(normalizePath("../man/figures/SCs_pre_bridging.pdf"), 
                        error = FALSE)
```
...
```{r echo=FALSE, fig.cap=f9, fig.pos="H"}
knitr::include_graphics(normalizePath("../man/figures/bridges_pre_bridging.pdf"),
                        error = FALSE)
```
...
``` {r pca_post_SC, eval=FALSE, echo = FALSE}
...
```{r echo=FALSE, fig.cap=f10, fig.pos="H", eval=FALSE}
knitr::include_graphics(normalizePath("../man/figures/SCs_post_bridging.pdf"),
                        error = FALSE)
```
...
```{r echo=FALSE, fig.cap=f11, fig.pos="H"}
knitr::include_graphics(normalizePath("../man/figures/bridges_post_bridging.pdf"), 
                        error = FALSE)
```
