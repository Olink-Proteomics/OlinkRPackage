...
```{r include=FALSE}
data_explore3072 <- OlinkAnalyze:::data_3k
data_exploreht <- OlinkAnalyze:::data_ht
```
...
```{r include=FALSE}
overlapping_samples <- unique(intersect(data_explore3072_samples,
                                        data_exploreht_samples))[1:50]
# Only use top 48 as overlapping samples
```
...
```{r, eval=FALSE}
# Find shared samples
npx_1 <- data_exploreht |>
  dplyr::mutate(Project = "data1") 
npx_2 <- data_explore3072 |>
...
```{r, fig.cap= fcap, eval=TRUE, echo=FALSE, out.width="100%"}
knitr::include_graphics(normalizePath("../man/figures/assay_bridgeability.png"), 
                        error = FALSE)
...
```{r eval=FALSE, include=FALSE, post_bridging_3k_data , echo=FALSE}
npx_br_data |> 
  head(10)
  kableExtra::kbl(booktabs = TRUE,
      digits = 1,
...
```{r pca_pre_sc, message=FALSE, fig.cap= f8, eval=FALSE, echo = TRUE}
## Before Bridging
npx_br_data |> 
  dplyr::filter(SampleType == "SAMPLE_CONTROL") |> 
  dplyr::mutate(OlinkID = paste0(OlinkID, "_", OlinkID_OlinkID_E3072)) |> 
  dplyr:::mutate(SampleID = paste0(Project, SampleID)) |> 
  OlinkAnalyze::olink_pca_plot(color_g = "Project")
```

```{r pca_pre_bridge, message=FALSE, fig.cap= f9, eval=FALSE, echo = TRUE}
## Before Bridging
npx_br_data |> 
  dplyr::filter(SampleType == "SAMPLE") |> 
...
``` {r pca_post_prep, message=FALSE, eval=FALSE, echo = TRUE}
## After bridging PCA
...
```
``` {r pca_post_SC, message=FALSE, eval=FALSE, echo = TRUE}
...
``` {r pca_post_bridge, message=FALSE, eval=FALSE, echo = TRUE}
