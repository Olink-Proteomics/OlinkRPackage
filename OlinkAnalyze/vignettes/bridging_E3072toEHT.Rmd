...
Individual Olink^®^ NPX^TM^ projectss are generally normalized using
...
number of assays being measured, the reagents being used, and other such differences, can sometimes lead to signal in one product and noise in
...
data and Explore HT data which has at least 40 to 64 bridging samples. For
...
tutorial](https://cran.r-project.org/web/packages/OlinkAnalyze/vignettes/bridging_introduction.html) or otherwise normalized together
...
In a case where the same sample have different sample IDs in the two projects, the `overlap_sample_list`
...
data.frame(SampleID = intersect(data_E3072$SampleID, data_EHT$SampleID)) |>
  ...
  kableExtra::kbl(booktabs = TRUE,
      digits = 2,
      caption = "Table 3. Overlapping bridging samples") |>
