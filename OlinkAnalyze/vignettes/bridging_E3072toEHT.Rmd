...
data.frame(SampleID = intersect(data_E3072$SampleID, data_EHT$SampleID)) |>
  dplyr::filter(SampleType == "SAMPLE") # Note that if `SampleType` is not is input data, stringr::str_detect can be used to exclude control samples based on naming convention.
```
...
npx_before_br <- data_E3072 |>
  ...
  dplyr::filter(SampleType == "SAMPLE") # Note that if `SampleType` is not is input data, stringr::str_detect can be used to exclude control samples based on naming convention.
      ...
      dplyr::mutate(SampleID = if_else(SampleID %in% overlapping_samples,
...
### PCA plot of the data from SCs
  npx_after_br_final |> dplyr::filter(SampleType == "SAMPLE")) |> 
    OlinkAnalyze::olink_pca_plot(color_g     = "Type")
  
### PCA plot of the data from bridging samples
  npx_after_br_final |> 
    dplyr::filter(stringr::str_detect(Type, "Bridge")) |> 
    OlinkAnalyze::olink_pca_plot(color_g     = "Type")
```
