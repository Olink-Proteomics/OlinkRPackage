...
![Schematic of Explore 3072 to Explore HT Bridging
Workflow](../man/figures/Bridging%20schematic%20for%20Tutorial.png)
...
![Criteria to determine the recommended bridging method for an assay (if any)](../man/figures/assay_bridgeability.png)

Prior to assessment, outlier bridge samples are excluded. A sample is considered an outlier if the NPX value is more than 3 * the interquartile range above or below the median on either platform. 

After assessment, an assay is considered bridgeable if it meets the first three criteria. The fourth criteria determines which normalizaiton method is recommended for bridging. If all four criteria are met than the recommended method is normalization using the median of paired differences. If only the first three criteria are met then quantile-quantile normalization is recommended. If any of the first three criteria are not met than bridging is not recommended for that assay. Note that bridgeable assays
