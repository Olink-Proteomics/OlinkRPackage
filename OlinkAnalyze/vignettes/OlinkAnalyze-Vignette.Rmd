...
> This vignette provides an overview of the main functions in OlinkAnalyze,
...
data. Running `check_npx()` directly is optional for most workflows: imported
...
OlinkAnalyze can store the output of `check_npx()` directly on the NPX data
...
before passing the result to another OlinkAnalyze function. When
`check_log = NULL`, OlinkAnalyze first tries to use the attached `check_log`;
if none is present, it runs `check_npx()` internally.

If you manually edit an Olink dataset outside the OlinkAnalyze preprocessing
...
If no `check_log` is supplied, `clean_npx()` first uses an attached `check_log`
from the input data and otherwise runs `check_npx()` internally.
