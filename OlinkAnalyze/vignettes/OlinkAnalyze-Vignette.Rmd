...
-   `OlinkID` *\<chr\>*: Unique ID for each assay assigned by Olink. In case
    the assay is included in more than one panels it will have a different
    `OlinkID` in each one.
...
The `read_npx()` function imports an NPX file into a tidy format to work with in
...
-   `OlinkID`: Unique ID for each assay assigned by Olink. In case the assay is
    included in more than one panels it will have a different `OlinkID` in each
    one.
-   `UniProt`: UniProt ID.
...
their data. **Note** that for the time being, not all functions utilize the full
...
downstream analysis. An schematic illustration of the described workflow is
...
missingness \<10% and for multi-plate projects, and for missingness \<5% for
...
A list of objects of class `ggplot2` is silently returned. Plots are also
printed unless option `quiet = TRUE` is set.
...
plot2 <- data_clean |>
  tidyr::drop_na() |> # removing missing values which exists for Site
  OlinkAnalyze::olink_boxplot(
    variable = "Site",
    olinkid_list = c("OID00488", "OID01276"),
    number_of_proteins_per_plot = 2L,
...
A list of objects of class `ggplot2`.
...
A bar graph as a `ggplot2` object.
