...
> **Note:** Detailed tutorial vignettes from OlinkAnalyze are now hosted in the
> **OlinkAnalyzeVignettes** package, which will be published on CRAN soon. Please
> look for **OlinkAnalyzeVignettes** for in-depth tutorials on topics such as
> plate randomization, bridging, LOD calculation, outlier exclusion, and more.
...
-   *check_npx* Function to check NPX data format and identify potential issues
-   *clean_npx* Function to clean NPX data based on checks from check_npx
...
The check_npx function performs various checks on NPX data, including checking column names, validating Olink identifiers, identifying assays with *NA* values for all samples and detecting duplicate sample identifiers. OlinkAnalyze uses pre-defined names of columns to perform downstream analyses, and different Olink platforms export data with different column names. This function aims to instruct each function of OlinkAnalyze on the column it should be using for the downstream analysis.

The output of check_npx (referred to as `check_log`) can be passed to all subsequent OlinkAnalyze functions that accept the `check_log` parameter. This avoids redundant checks and improves performance when running multiple analyses on the same data.

### Function arguments

-   df: NPX data frame in long format (e.g. as returned by read_NPX).
-   preferred_names: (Optional) Named character vector to map custom column names to internal names (e.g. `c("quant" = "PCNormalizedNPX")`).

```{r message=FALSE, eval=FALSE}
# Run check_npx on the data
npx_check <- check_npx(df = npx_data1)
```
...
-   col_names: Mapped column names from the input data.
-   oid_invalid: Invalid OlinkID values.
-   assay_na: Assays with all NA quantification values.
-   sample_id_dups: Duplicate SampleIDs.
-   sample_id_na: Samples with all NA quantification values.
-   col_class: Columns with incorrect data types.
-   assay_qc: Assays with QC warnings.
-   non_unique_uniprot: OlinkIDs mapping to multiple UniProt IDs.
-   darid_invalid: Outdated DataAnalysisRefID/PanelDataArchiveVersion combinations.

## Clean NPX data (clean_npx)

The clean_npx function applies a series of cleaning steps to a data set exported by Olink Software and imported in R by read_NPX. Some of the steps rely on results from check_npx (passed via `check_log`). This function removes samples and assays that are not suitable for downstream statistical analysis.

### Function arguments

-   df: NPX data frame in long format.
-   check_log: (Optional) A named list returned by check_npx. If NULL, check_npx will be run internally.
-   remove_assay_na: Logical. Remove assays with all NA quantification values. Default: TRUE.
-   remove_invalid_oid: Logical. Remove assays with invalid OlinkIDs. Default: TRUE.
-   remove_dup_sample_id: Logical. Remove duplicate SampleIDs. Default: TRUE.
-   remove_control_sample: Logical. Remove external control samples. Default: TRUE.
-   remove_control_assay: Logical. Remove internal control assays. Default: TRUE.
-   remove_qc_warning: Logical. Remove samples failing QC. Default: TRUE.
-   remove_assay_warning: Logical. Remove assays with QC warnings. Default: TRUE.
-   convert_df_cols: Logical. Correct column data types. Default: TRUE.
-   convert_nonunique_uniprot: Logical. Resolve multiple UniProt mappings per assay. Default: TRUE.
-   verbose: Logical. Print information about cleaning steps. Default: FALSE.

```{r message=FALSE, eval=FALSE}
# Clean NPX data using the check_log from check_npx
npx_clean <- clean_npx(df = npx_data1,
                        check_log = npx_check)
```

### Function output

A tibble with cleaned NPX data, where problematic samples and assays have been removed and column types corrected according to the specified cleaning parameters.
...
For more information on plate randomization, consult the Plate Randomization Vignette in the **OlinkAnalyzeVignettes** package.
...
For more information on bridge sample selection, consult the Introduction to bridging Olink® NPX datasets tutorial in the **OlinkAnalyzeVignettes** package.
...
-   **Bridging normalization**: One of the dataframes is adjusted to another using overlapping samples (bridge samples). The overlapping samples should have the same IDs between dataframes. Adjustment is made using the median of the paired differences between the bridge samples. For more information on bridging, consult the Introduction to bridging Olink® NPX datasets tutorial in the **OlinkAnalyzeVignettes** package.
...
For more information on calculating LOD, consult the Calculating LOD from Olink® Explore data tutorial in the **OlinkAnalyzeVignettes** package.
...
# Run check_npx on cleaned data
npx_check <- check_npx(df = npx_data1_no_controls)
...
More information about `olink_pca()` can be found in the Outlier Exclusion Vignette in the **OlinkAnalyzeVignettes** package.
...
More information about `olink_dist_plot()` can be found in the Outlier Exclusion Vignette in the **OlinkAnalyzeVignettes** package.
...
More information about `olink_qc_plot()` can be found in the Outlier Exclusion Vignette in the **OlinkAnalyzeVignettes** package.
...
npx_small_check <- check_npx(df = npx_data_small)
