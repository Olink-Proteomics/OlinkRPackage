...
-   `clean_npx()` Function to clean NPX data based on the output of
    `check_npx()`
-   `olink_extract_check_log()` Function to retrieve an attached `check_log`
...
NPX data from `read_npx()`, as well as NPX data exported from `clean_npx()`, `olink_lod()` and the normalization functions (e.g. `olink_normalization()`, `olink_normalization_bridge()` and `olink_normalization_subset()`), already carries a `check_log`, and downstream Olink
Analyze functions can use either an attached `check_log`, a user-supplied
`check_log` argument, or an internally generated one. Calling `check_npx()`
explicitly is still useful when you want to inspect the checks yourself, work
with manually created or modified NPX data, or define custom column mappings
through `preferred_names`.
...
-   `olink_extract_check_log()` retrieves the attached `check_log` from an
...
check_log <- OlinkAnalyze::olink_extract_check_log(
  df = data
)
...
[Plate Randomization Vignette](https://CRAN.R-project.org/package=OlinkAnalyzeVignettes/vignettes/plate_randomizer.html) in the R package `OlinkAnalyzeVignettes`.
...
[Outlier Exclusion Vignette](https://CRAN.R-project.org/package=OlinkAnalyzeVignettes/vignettes/outlier_exclusion.html) in the R package `OlinkAnalyzeVignettes`.
