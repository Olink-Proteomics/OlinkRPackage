...
npx_data1_no_controls <- OlinkAnalyze::npx_data1 |>
  dplyr::filter(
    !stringr::str_detect(
      string = .data[["SampleID"]],
      pattern = regex("control|ctrl", ignore_case = TRUE)
    )
  ) |>
  dplyr::filter(
    !stringr::str_detect(
...
```{r message=FALSE, eval=FALSE}
first10 <- OlinkAnalyze::npx_data1 |>
  dplyr::pull(
    .data[["OlinkID"]]
  ) |>
  unique() |>
...
first15samples <- OlinkAnalyze::npx_data1 |>
  dplyr::pull(
    .data[["SampleID"]]
  ) |>
  unique() |>
...
npx_data_small <- OlinkAnalyze::npx_data1 |>
  dplyr::filter(
    !stringr::str_detect(
      string = .data[["SampleID"]],
      pattern = "CONT"
