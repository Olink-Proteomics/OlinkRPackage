...
# Read NPX file from package example data
...
check_npx_result <- OlinkAnalyze::check_npx(df = npx_df)
```
...
A tibble (or data.frame) in long format containing the cleaned NPX data, with
...
check_npx_result <- OlinkAnalyze::check_npx(df = OlinkAnalyze::npx_data1)
npx_clean <- OlinkAnalyze::clean_npx(
...
```{r message=FALSE, eval=FALSE}
# Run check_npx and clean_npx before analysis (removes control samples/assays)
check_npx_result <- OlinkAnalyze::check_npx(df = OlinkAnalyze::npx_data1)
npx_clean <- OlinkAnalyze::clean_npx(
  df = OlinkAnalyze::npx_data1,
  check_log = check_npx_result
...
# Run check_npx and clean_npx before statistical analysis
check_npx_result <- OlinkAnalyze::check_npx(df = npx_df)
npx_df_clean <- OlinkAnalyze::clean_npx(
  df = npx_df,
  check_log = check_npx_result
)
