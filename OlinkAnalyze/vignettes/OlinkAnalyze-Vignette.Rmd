...
`check_npx` and attaches the results to the returned `olink_npx` object. This
...
```{r message=FALSE, eval=FALSE}
# read_npx now returns an olink_npx object with check_log attached
...
-   check_log: **Deprecated.** Named list returned by `check_npx`. The check
    log is now automatically extracted from the `olink_npx` object returned
    by `read_npx`. You only need to pass this if working with a plain tibble
    or data frame that is not an `olink_npx` object.
...
```{r message=FALSE, eval=FALSE}
# Clean the NPX data — check_log is extracted automatically from olink_npx
...
A tibble (or ArrowObject) of class `olink_npx` in long format containing the
...
-   check_log: **Deprecated.** Named list returned by `check_npx`. The check
log is now automatically extracted from the `olink_npx` object. Passing this
