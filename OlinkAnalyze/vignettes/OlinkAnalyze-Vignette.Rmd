...
imported with `read_npx`. Starting from v5.0, `read_npx` automatically runs
`check_npx` and attaches the results to the returned `olink_class` object. This
means you no longer need to call `check_npx` separately for routine workflows.
However, you can still call it explicitly to inspect the results or to pass
custom `preferred_names`.
...
```{r message=FALSE, eval=FALSE}
# read_npx now returns an olink_class object with check_log attached
npx_data <- OlinkAnalyze::read_npx(
  filename = "path/to/npx_file.parquet"
)

# Access the check log
check_log <- OlinkAnalyze::olink_check_log(npx_data)

# Or run check_npx explicitly for inspection or with preferred_names
...
After cleaning, `clean_npx` automatically re-runs `check_npx` on the output
to produce an updated check log, ensuring the attached metadata reflects the
cleaned state of the data.

**Note:** Downstream analysis functions (e.g., `olink_ttest`, `olink_anova`)
now automatically detect if the data needs cleaning and will run `clean_npx`
internally if necessary. If you prefer to control the cleaning process,
call `clean_npx` explicitly before analysis.

### Function arguments

-   df: NPX data frame in long format as returned by `read_npx`.
-   check_log: **Deprecated.** Named list returned by `check_npx`. The check
    log is now automatically extracted from the `olink_class` object returned
    by `read_npx`. You only need to pass this if working with a plain tibble
    or data frame that is not an `olink_class` object.
...
```{r message=FALSE, eval=FALSE}
# Clean the NPX data — check_log is extracted automatically from olink_class
npx_clean <- OlinkAnalyze::clean_npx(
  df = npx_data
)

# Or with explicit check_log for backward compatibility
npx_clean <- OlinkAnalyze::clean_npx(
  df = OlinkAnalyze::npx_data1,
...
A tibble (or ArrowObject) of class `olink_class` in long format containing the
cleaned NPX data, with invalid assays, control samples, QC-failing samples,
and problematic entries removed according to the chosen arguments. The attached
check log reflects the state of the cleaned data (i.e., `check_npx` is
automatically re-run on the output).
...
-   check_log: **Deprecated.** Named list returned by `check_npx`. The check
log is now automatically extracted from the `olink_class` object. Passing this
explicitly is supported for backward compatibility but no longer necessary.

```{r message=FALSE, eval=FALSE}
# New workflow: just pass the cleaned data — check_log is attached
