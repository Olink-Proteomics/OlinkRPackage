...
*Some assays will use count values as the LOD because the assays receive
very few counts in the negative controls. For the convenience of data
processing, these LOD in count values are converted to NPX values in the
`olink_lod()` function. In this process the LOD value for this assay (in
counts) will become many LOD values in NPX (as extension control counts
will vary across all samples). This is because when counts are small, a
small change on the count scale can be a large change on the NPX scale.
This is because NPX is a relative scale and is based on the ratio of the
counts of the assay to the counts of the extension control. For example,
