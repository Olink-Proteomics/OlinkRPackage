...
When using the `olink_normalization_bridge()` function to bridge two datasets, the reference project remains unchanged throughout the bridging procedure. Both the NC LOD calculated from negative controls in the reference project and the fixed LOD attached to the reference project stay the same. In contrast, the LOD values (both NC LOD and fixed LOD) for the non-reference project are adjusted using an adjustment factor calculated from the median of the paired NPX LOD differences per assay across the bridging samples.
