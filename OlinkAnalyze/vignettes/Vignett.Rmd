...
The olink_normalization_n is a function used to remove batch effect from multiple runs. This function is mainly used if the number of projects are three or more, otherwise please see the olink_normalization function. Commonly, there is a shift in NPX values between runs, however, the spread of the data remains the same. This is why normalization between dataframes/projects is required. For each dataframe/project another dataframe/project will be used as a reference to normalize to. The function handles two different types of normalization, Bridge and Subset, which are explained in the olink_normalization section.
