...
olink_normalization(df1 = npx_data1, 
                    df2 = npx_data2, 
                    overlapping_samples_df1 = fullsubset_df1,
                    overlapping_samples_df2 = fullsubset_df2,
                    df1_project_nr = '20200001',
                    df2_project_nr = '20200002',
                    reference_project = '20200001')

# Example of using a subset samples for normalization
...
# Example of using a reference median for normalization

ref_df <- npx_df2 %>%
  group_by(OlinkID) %>%
  summarise(Reference_NPX = median(NPX)) 

olink_normalization(df1 = npx_df1,
                    overlapping_samples_df1 = subset_df1,
                    reference_medians = ref_df)
```  
...
## Normalizing muliple NPX data (olink_normalization_n)  

The olink_normalization_n is a function used to normalize NPX values between multiple different dataframes/projects which have been ran at different times all at the same time. This function is mainly used if the number of projects are three or more, otherwise please see the olink_normalization function. Commonly, there is a shift in (mean) NPX values between runs, however, the spread of the data remains the same. This is why normalization between dataframes/projects is required. For each dataframe/project another dataframe/project will be used asa reference to normalize to. The function handles two different types of normalization, Bridge and Subset, which are explained in the olink_normalization section.


The input of this function is a tibble that contains all the necessary information to normalize multiple NPX projects. This tibble is called the normalization schema. The basic idea is that every row of the data frame is a separate project to be normalized. It is assume that there is always one baseline project that does not normalize to any other. All other project normalize to one or more projects. The function handles projects that are normalized in a chain, for example:

- project 2 normalizes to project 1, and project 3 normalizes to project 2.

- project 2 normalizes to project 1, and project 3 normalizes to the combined data frame of projects 1 and 2 (that is already normalized).
...
# Creating NPX data frames to bridge 
npx_df1 <- npx_data1 %>% 
  filter(!stringr::str_detect(SampleID, "CONTROL_")) %>%
  select(-Project) %>%
...
# manipulating the sample NPX datasets to create another two random data frames
npx_df3 <- npx_data2 %>%
  mutate(SampleID = paste0(SampleID, "_mod"),
                PlateID = paste0(PlateID, "_mod"),
                NPX = sample(x = NPX, size = n(), replace = FALSE)) %>%
  filter(!stringr::str_detect(SampleID, "CONTROL_")) %>%
  select(-Project) %>%
  mutate(Normalization = "Intensity")

npx_df4 <- npx_data1 %>%
  mutate(SampleID = paste0(SampleID, "_mod2"),
                PlateID = paste0(PlateID, "_mod2"),
                NPX = sample(x = NPX, size = n(), replace = FALSE)) %>%
  filter(!stringr::str_detect(SampleID, "CONTROL_")) %>%
  select(-Project) %>%
  mutate(Normalization = "Intensity")
...
overlap_samples <- intersect(unique(npx_df1$SampleID),
...
## Samples to use for subset normalization between npx_df4 and the
