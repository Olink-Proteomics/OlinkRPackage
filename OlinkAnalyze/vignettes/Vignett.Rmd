...
header-includes:
  ...
  - \fancyhead[C]{\textbf{Olink® Analyze Vignette}}
  - \fancyhead[R]{\thepage}
...
data <- read_NPX("~/path/to/NPX_file.parquet")
```
...
data <-  list.files(path = "path/to/dir/with/NPX/files",
    ...
    OlinkAnalyze::read_NPX(x) |> 
      ...
      dplyr::mutate(File = x) 
         ...
         # optionally return one data frame instead of a list of data frames
         dplyr::bind_rows() 
...
anova_posthoc_oneway_results <- 
  olink_anova_posthoc(df = npx_data1_no_controls,
...
## Uniform Manifold Approximation and Projection (UMAP) plot (olink_umap_plot)
...
npx_data1 |> 
  ...
  olink_umap_plot(color_g = "QC_Warning", byPanel = TRUE)  
  }
```

```{r message=FALSE, eval=FALSE, echo = FALSE, out.width = "\\linewidth"}
...
**Note:** Plots will not appear in the *plots* panel of *Rstudio* unless they are assigned to a variable and explicitly printed (see sample code above).
...
```{r message=FALSE, fig.width = 8,out.width = "\\linewidth"}
if (requireNamespace("lme4", quietly = TRUE) &
    requireNamespace("lmerTest", quietly = TRUE)) {
...
```{r message=FALSE, eval = FALSE, fig.width = 8, echo = FALSE, out.width = "\\linewidth"}
...
```{r  message=FALSE, fig.height=4,fig.width=8, out.width="\\linewidth"}
...
```{r message=FALSE, fig.width=8, fig.height=6, out.width="\\linewidth"}
first10 <- npx_data1 |>
  dplyr::pull(OlinkID) |> 
  unique() |> 
  head(10)
...
```{r message=FALSE, eval=FALSE, fig.height=4, echo = FALSE, out.width = "\\linewidth"}
...
```{r message=FALSE, eval = TRUE, out.width="\\linewidth"}
# perform t-test
...
```{r message=FALSE, echo = FALSE, out.width = "\\linewidth"}
