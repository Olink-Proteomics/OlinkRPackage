...
anova_results_oneway <- olink_anova(
  df = npx_data1,
  variable = "Site"
)
# Two-way ANOVA, no covariates
anova_results_twoway <- olink_anova(
  df = npx_data1,
...
anova_results_oneway_significant <- anova_results_oneway %>%
  ...
  pull(OlinkID)
anova_posthoc_oneway_results <- olink_anova_posthoc(
  df = npx_data1,
...
ordinalRegression_results_twoway <- olink_ordinalRegression(
  df = npx_data1,
  variable = c("Site", "Time")
)
# One-way ordinal regression, Treatment as covariates
ordinalRegression_oneway <- olink_ordinalRegression(
  df = npx_data1,
  variable = "Site",
  covariates = "Treatment"
)
...
lmer_results_oneway <- olink_lmer(
  df = npx_data1,
  variable = "Site",
  random = "Subject"
)
# Linear mixed model with two variables.
lmer_results_twoway <- olink_lmer(
  df = npx_data1,
...
lmer_posthoc_twoway_results <- olink_lmer_posthoc(
  df = npx_data1,
...
npx_data1 %>%
  filter(
    !str_detect(SampleID, "CONTROL_SAMPLE"),
...
qc$data %>%
  filter(Outlier == 1) %>%
  select(SampleID, Panel, IQR, sample_median, Outlier)
```
