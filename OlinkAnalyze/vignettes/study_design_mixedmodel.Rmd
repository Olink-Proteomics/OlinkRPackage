...
output: 
  html_vignette:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{A Guide to Comparative Analysis of longitudinal data across multiple groups}
...
# Argument should be the location of your NPX data. 
# Data are usually in parquet, csv, xlsx or zip format.
# Data file format depends on the product.
```  
...
lmer_posthoc_results_Time %>%
  ...
  knitr::kable(format = "html",
               ...
               longtable = T,
               row.names = F,
               caption = "**Table 4.** ANOVA post hoc results of the 5 most significant contrasts and proteins. Some columns have been left out due to space.",
               label = "manifest")
```  
...
A point range plot is a common way to illustrate the results from an LMM. These visualizations provide valuable insights into the differences between the groups and help in understanding the significance and effect sizes of the observed effects.
