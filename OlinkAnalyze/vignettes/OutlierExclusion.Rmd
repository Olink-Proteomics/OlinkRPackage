...
header-includes:
  ...
  - \fancyhead[L]{\textbf{Outlier Exclusion Vignette}}
  # - \fancyhead[R]{\thepage}
...
This vignette describes how to use Olink® Analyze to evaluate a dataset for the presence of outliers. When performing statistical analyze it is important to determine if there are any outlier samples in the data prior to statistical analysis. There are many reasons why a sample might be an outlier; it could be due to a data entry or measurement error (for example, labeling a control sample as a disease sample), sampling problems or unusual conditions (for example, contamination), or natural variation. In many parametric statistics tests the mean of each group is used to determine differences between groups. Since the mean is highly sensitive to outliers, it is important to examine datasets for outliers prior to analysis as these outliers could have a large influence on the statistical results.
...
ggpubr::ggarrange(p1[[1]], p2[[1]], nrow = 1, labels = "AUTO")
```

```{r Outlier_Example, echo=FALSE, fig.cap=fcap, out.width="\\linewidth"}
# knitr::include_graphics(normalizePath("../man/figures/PCA_Outlier_Fig1.png"),error = FALSE)
p1 <-
  {
    outlier_data |> olink_pca_plot(label_samples = T, quiet = T)
  }[[1]] +
  ggplot2::theme(
...
p2 <-
  {
    group_data |>
      olink_pca_plot(color_g = "Site", quiet = T)
  }[[1]] +
  ggplot2::theme(
    legend.text = ggplot2::element_text(size = 7),
    legend.title = ggplot2::element_text(size = 9),
...
ggpubr::ggarrange(p1, p2, nrow = 1, labels = "AUTO", legend = "bottom", widths = c(1,1.1)) 
fcap <- "\\textbf{A.} PCA can be used to identify individual outlier samples as shown by samples A25 and A52. \\textbf{B.} PCA can be used to identify difference in groups as seen by Site\\_D samples. This is not suggesting Site D is an outlier, but rather that there may be a global difference between sites."
```
...
OlinkAnalyze::npx_data1 |> 
  dplyr::filter(
    stringr::str_detect(SampleID, "CONTROL", negate = T)) |> # Remove control samples
...
pca_plots <- OlinkAnalyze::npx_data2 |>         # Save the PCA plot to a variable
  dplyr::filter(                                # Remove control samples
    stringr::str_detect(SampleID, "CONTROL", negate = T)
  ) |> 
...
outlier_data |> 
  dplyr::filter(
    stringr::str_detect(SampleID, "CONTROL", negate = T)) |> # Remove control samples
  olink_pca_plot(label_samples = FALSE, byPanel = TRUE ) 
```
...
```{r, eval = FALSE}
outlier_data |> 
  dplyr::filter(SampleID %in% c("A25", "A52", "A1", "A2", "A3", "A5", 
                                "A15", "A16", "A18", "A19", "A20"))|> 
  olink_dist_plot()
```
...
```{r, eval=FALSE}
group_data |> 
  dplyr::filter(Site %in% c("Site_A", "Site_D")) |> # Visualizing 2 sites to see all samples
