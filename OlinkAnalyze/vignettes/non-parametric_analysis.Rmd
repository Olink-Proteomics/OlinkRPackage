...
Mann-Whitney U Test (Wilcoxon Rank-Sum Test): This test is used to compare two independent groups to determine if there is a significant difference between them.

Wilcoxon Signed-Rank Test: When dealing with two related (paired) samples, this test is used to determine if there is a significant difference between them.
...
In the following examples, we will demonstrate how to perform different non-parametric tests using the dataset `npx_data1` which will act as the reference data. 
